...
with sync_playwright() as playwright:
    ...
    page.locator("#recipe-search").fill("110 in the shade")
    page.wait_for_timeout(250)
    shade_card = page.locator('.recipe-card:has(.recipe-open[data-id="15423"])')
    shade_card.locator(".recipe-open").click()
    expect(page.locator(".instruction")).to_contain_text("shot 杯")
    expect(page.locator(".instruction")).not_to_contain_text("射手")
    expect(page.locator(".ingredient-list")).to_contain_text("拉格啤酒")
    expect(page.locator(".ingredient-list")).to_contain_text("龙舌兰酒")
    ...
    page.locator('[data-lang="zh-TW"]').click()
    expect(page.locator("html")).to_have_attribute("lang", "zh-TW")
    expect(page.locator("#hero-title")).to_contain_text("吧檯")
    expect(page.locator("#recipe-search")).to_have_attribute("placeholder", "例如：莫希托、Mojito、琴酒")
    expect(mojito_card.locator(".localized-name")).to_have_text("莫希托")
    mojito_card.locator(".recipe-open").click()
    expect(page.locator(".ingredient-list")).to_contain_text("白蘭姆酒")
    expect(page.locator(".ingredient-list")).to_contain_text("萊姆")
    expect(page.locator(".instruction")).to_contain_text("萊姆")
    page.get_by_role("button", name="關閉").click()
    page.screenshot(path=str(SCREENSHOT_DIR / "recipes-zh-tw.png"), full_page=True)
    ...
    expect(order_mojito.locator(".poem-original")).not_to_be_empty()
    expect(order_mojito.locator(".poem-translation")).not_to_be_empty()
    expect(order_mojito.locator(".drink-poem figcaption a")).to_contain_text("Verify the poem")
    ...
    expect(page.locator(".selected-drink .localized-name")).to_have_text("莫希托")
    expect(page.locator(".drink-poem figcaption a").first).to_contain_text("核對原作")
    expect(page.locator(".order-card-ingredients").first).to_contain_text("白蘭姆酒")
    expect(page.locator(".order-card-ingredients").first).to_contain_text("萊姆")
    page.wait_for_timeout(2200)
    page.screenshot(path=str(SCREENSHOT_DIR / "order-zh-tw.png"), full_page=True)
    ...
    expect(page.locator(".drink-poem figcaption a").first).to_contain_text("核对原作")
    page.locator("#order-search").fill("110 in the shade")
    page.wait_for_timeout(250)
    shade_order_card = page.locator('.order-menu-item:has([data-add-id="15423"])')
    expect(shade_order_card.locator(".order-card-ingredients")).to_contain_text("拉格啤酒 · 龙舌兰酒")
    assert_no_horizontal_overflow(page)
    ...
    print("PASS: Simplified + Traditional Chinese, contemporary bar terms, English-only names, persisted locale, localized order flow, and mobile overflow")
